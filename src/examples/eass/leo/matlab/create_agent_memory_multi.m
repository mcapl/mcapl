...
%Function to globally initialise/declare multi-agent memory fields

function create_agent_memory_multi(N)

global ClusterMemory;

for i=1:N,
	aname=sprintf('Agent%d',i);
	ClusterMemory.(aname)=retrieve_memory_multi(N);
end    

%initialize sockets on physical
InitializePhysical;