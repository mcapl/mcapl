...
package gwendolen.ajpf_tutorials.tutorial3;
...
public class SearchAndRescueDynamicEnv extends DefaultEnvironment implements
		MCAPLJobber {
	static final String logname = "gwendolen.ajpf_tutorials.tutorial3.SearchAndRescueDynamicEnv";
...
/*	public Human human_at_location() {
	...
	public boolean done() {
		return humans.isEmpty() || (robot_x == 2 && robot_y == 2);
	}
	...
	public void injured_humans_in_area() {
		Predicate injured_humans = new Predicate("injured_humans");
		for (Human h: humans) {
			if (h.injured()) {
				addPercept(injured_humans);
				return;
			}
		}
		
		removePercept(injured_humans);
	}
