...
/**
 * Abstract version of an interface for References to logical formulae representing implications 
 * that appear in Guards.
 * @author lad
 *
 */
public class Abstract_GuardImplication implements Abstract_GLogicalFormula, Abstract_LogicalFormula {
	Abstract_LogicalFormula pre;
	Abstract_LogicalFormula post;
	
	public Abstract_GuardImplication(Abstract_LogicalFormula p, Abstract_LogicalFormula pt) {
		pre = p;
		post = pt;
	}
	

	public Implication toMCAPL() {
		return new Implication(pre.toMCAPL(), post.toMCAPL());
	}

	public int newJPFObject(MJIEnv env) {
    	int objref = env.newObject("ail.syntax.ast.Abstract_GuardImplication");
