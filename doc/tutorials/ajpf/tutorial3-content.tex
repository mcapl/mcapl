...
This tutorial assumes familiarity with the operation of \ajpf\ as described in \ajpf\ tutorials 1 and 2 and familiarity with the theory of model checking.  Unlike most tutorials, this tutorial is not standalone and assumes the user has access to both \spin\ and \prism\index{SPIN}\index{Prism}.
...
We will use the same program that was used in \ajpf\ tutorial 2.  In the directory for tutorial 3 you will find the configuration file \texttt{TwoPickUpAgents\_Spin.jpf}\index{TwoPickUpAgents\_Spin.jpf} that is shown in figure~\ref{fig:spin_config}.
...
\paragraph{Printing the Output to a file} You can obviously cut and paste the \promela\ model\idnex{Promela} from the \ajpf\ output into a file for use with \spin.  Alternatively you can set \texttt{ajpf.model.location}\index{ajpf.model.location} to the path to an output file.  The path should be relative to your \texttt{HOME} directory.  
...
The file \texttt{TwoPickUpAgents\_SpinToFile.jpf}\index{TwoPickUpAgents\_SpinToFile.jpf} will print the model\index{model} to a file \texttt{tutorial3\_spin.pml} in the tutorial directory.
...
\item where to output the program model.  
\end{enumerate}
Because \prism\ also includes probabilistic information\index{Prism} in the model, when using \ajpf\ with \prism\ it is also important to use a \emph{listener}\index{JPF!listener} that records such information when a choice in the java execution is governed by a probability.

This is done in the \ajpf\ configuration file\index{configuration!JPF}\index{JPF!configuration}.  Initially we will, once again, use the same program that was used in \ajpf\ tutorial 2.  In the directory for tutorial 3 you will find the configuration file \texttt{TwoPickUpAgents\_Prism.jpf}\index{TwoPIckUpAgents\_Prism.jpf} that is shown in figure~\ref{fig:prism_config1}.
...
The file \texttt{TwoPickUpAgents\_PrismToFile.jpf}\index{TwoPickUpAgents\_PrismToFile.jpf} will print the model to a file \texttt{tutorial3\_prism.pm} in the tutorial directory.
