...
/**
 * FireVerificationEnvironment provides a verification environment - in the style
 * outlined in Practical Verification of Decision-Making in Agent-Based Autonomous Systems for
 * the example of a smart home that controls access to lights and evacuation 
 * procedures as outlined in Verifiable Machine Ethics in Changing Contexts.
 */
public class PrivacyVerificationEnv extends EASSVerificationEnvironment {
	public String logname = "juno.smarthome.FireVerificationEnvironment";
	...
	public Set<Predicate> generate_sharedbeliefs(String agName, Action act) {
		...
		if  (time_of_day == 0) {
			predicates.add(new Predicate("morning"));
			AJPFLogger.info(logname, "Morning");
		} else if (time_of_day == 1) {
			predicates.add(new Predicate("evening"));
			AJPFLogger.info(logname, "Evening");
		} else {
			predicates.add(new Predicate("afternoon"));
			AJPFLogger.info(logname, "Afternoon");
		}
		
		int  visitor = random_int_generator.nextInt(2);
		if (visitor == 0) {
			predicates.add(new Predicate("resident_with_visitors"));
			AJPFLogger.info(logname, "Visitors");
		}
		
		if (!notification) {
			int  reminder = random_int_generator.nextInt(2);
			if (reminder == 0) {
				Predicate set_reminder = new Predicate("set_reminder");
				
				int status = random_int_generator.nextInt(2);
				String s="private";
				if (status == 0) {
					set_reminder.addTerm(new Predicate("private"));
				} else {
					set_reminder.addTerm(new Predicate("public"));
					s = "public";
				}
				
				set_reminder.addTerm(new Predicate("evening"));
				AJPFLogger.info(logname, "Setting " + s + " reminder");
				predicates.add(set_reminder);
				notification = true;
			}
		}
