...
We will use the same program that was used in \ajpf\ tutorial 2.  In the directory for tutorial 3 you will find the configuration file \texttt{TwoPickUpAgents\_Spin.jpf}\index{example!SPIN}\index{example!twopickupagents} that is shown in figure~\ref{fig:spin_config}.
...
This is done in the \ajpf\ configuration file\index{configuration!JPF}\index{JPF!configuration}.  Initially we will, once again, use the same program that was used in \ajpf\ tutorial 2.  In the directory for tutorial 3 you will find the configuration file \texttt{TwoPickUpAgents\_Prism.jpf}\index{example!Prism}\index{example!twopickupagents} that is shown in figure~\ref{fig:prism_config1}.
...
In order for \ajpf's probability listener\index{JPF!listener}\index{JPF!listener!probabilistic}\index{MCAPLProbListener (class)} to work correctly, all randomness (and probabilistic behaviour) should be created using \ail's \texttt{Choice}\index{Choice (class)} classes as documented in \ail\ tutorial 3.  
