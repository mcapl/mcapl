...
%% \mid \phi \land \phi
  \phi \vee \phi \mid \phi \wedge \phi \mid \neg \phi
   \mid \next \phi \mid \phi \: \luntil \phi \mid \phi \release \phi \mid \sometime \phi \mid \always \phi
\end{array}
\end{equation*}
%
...
Note, in particular, that in property specification\index{property specification language} files ``not'' ($\sim$) must always appear in an innermost position next to one of the BDI agent properties such as $\texttt{B(}\AILagent\texttt{, }f\texttt{)}$\index{property!negated}  and the ``next'' operator ($\next$) must always appear in the next most innermost position.

It is possible to use $\phi \texttt{->} \psi$ as shorthand for $\neg \phi \vee \psi$ in property\index{property specification language} specification files.\index{property!implication}
...
$$
\begin{array}{lcl}
\MAS_i \models_{MC} \quad \next \: \varphi & \quad\text{iff}\quad&
if \MAS_{i+1} \models_{MC} \quad \varphi .\\[.5em]
%
