...
\end{verbatim}
\end{sloppypar}
\item Create a file \texttt{.jpf/site.properties} with the path to mcapl assigned to the value \texttt{mcapl} (e.g. \texttt{mcapl = \$\{user.home\}/Eclipse/mcapl}).
...
We also supply an eclipse \texttt{.project} file so you should be able to import ajpf2014 (or mcapl if using the development version) simply into eclipse.
\begin{itemize}
\item You should then be able to build ajpf by clicking on the 'build.xml file.
\item Create a file \texttt{.jpf/site.properties} with the path to ajpf2014 (or mcapl if using the development version)  assigned to the value \texttt{mcapl} (e.g. \texttt{mcapl = \$\{user.home\}/Eclipse/ajpf2014}).
