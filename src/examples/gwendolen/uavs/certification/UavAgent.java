...
/**
 * VEC123 executive with route planning. A copy of inputsensors7. 
 * 
 * 
 * 
 * 
...
====================================================== statistics
elapsed time:       1:43:01
states:             new=263, visited=239, backtracked=501, end=0
search:             maxDepth=32, constraints=0
choice generators:  thread=2, data=262
heap:               gc=237010, new=120583049, free=120538694
instructions:       448900660
max memory:         70MB
loaded code:        classes=281, methods=3802

====================================================== search finished: 8/10/12 7:45 PM
	...
	public static void main(String[] args) {
		...
		try {
			...
			masstring = "marija.UavAgentEnv " +
					...
					// if we want to complete mission, but we don't know flight phase, then ask vehicle for flight phase
					"+!missionComplete [achieve] : {B flightPhase(unknown), ~B connectionOk} <- send(env,connect); " +
										
					// if connection is ok, then do a poll cycle
					"+connectionOk : {~B sent(veh,reqfp), B flightPhase(unknown)} <- +.lock, +polled(self), -.lock; " +
					
					// if polled self, and flightphase is unknown, then request flight phase and poll
					"+poll(self) : {~B sent(veh,reqfp), B flightPhase(unknown)} <- +.lock, +sent(veh,reqfp), send(veh,reqfp), -poll(self), +polled(self), -.lock; " +
					...
					"+poll(self)  : {B veh(status,waitingAtRamp), B flightPhase(unknown), B sent(veh,reqfp)} <- +.lock, -veh(status,waitingAtRamp), -poll(self), -sent(veh,reqfp), -flightPhase(unknown), +flightPhase(waitingAtRamp), send(fuel,reqfuelstatus), +waiting(fuel,reqfuelstatus), +polled(self), -.lock; " +
					// .. but if we haven't found out yet then poll again.
					"+poll(self)  : {~B veh(status,X), B flightPhase(unknown), B sent(veh,reqfp)} <- +.lock, -poll(self), +polled(self), -.lock; " +
					
					// if we're polled, and we are waiting for fuel status and we have received the fuel status, then request position
					"+poll(self) : { B waiting(fuel,reqfuelstatus), B fuel(level,F) } <- +.lock, -poll(self), -waiting(fuel,reqfuelstatus), send(nav,reqposition), +waiting(nav,reqposition), +polled(self), -.lock; " +
					// .. but if we're still waiting for the fuel status then poll again. 
					"+poll(self) : { B waiting(fuel,reqfuelstatus), ~B fuel(level,F) } <- +.lock, -poll(self), +polled(self), -.lock; " +
					
					// if polled, and we are waiting for position and have received the position, then request route from planner and poll again
					"+poll(self) : {B position(Lat,Lon,Alt), B fuel(level,F), B waiting(nav,reqposition)} <- +.lock, -poll(self), -waiting(nav,reqposition),  send(atc,reqtc), +waiting(atc,tc), +polled(self), -.lock; " +
					// .. but if we're still waiting for a position, poll again.
					"+poll(self) : {~B position(Lat,Lon,Alt), B fuel(level,F), B destination(D), B waiting(nav,reqposition)} <- +.lock, -poll(self), +polled(self), -.lock; " +
					
					// if we're polled, and waiting at ramp and taxi clearance is given, then request a plan to the location R specified by ATC
					"+poll(self) : {B flightPhase(waitingAtRamp), B atc(tcg,R), B waiting(atc,tc), B position(Lat,Lon,Alt), B fuel(level,F)} <- +.lock, -poll(self), -waiting(atc,tc), send(planner,reqTaxiRoute,R,Lat,Lon,F), +waiting(planner,reqTaxiRoute), +polled(self), -.lock; " +
					// if we're polled and taxi clearance is denied, then request taxi clearance again and poll sensors
					"+poll(self) : {B flightPhase(waitingAtRamp), B atc(tcd), B waiting(atc,tc)} <- +.lock, -poll(self), -atc(tcd), send(atc,reqtc), +polled(self), -.lock; " +
					// if we're polled and we haven't heard back from atc, then poll sensors
					"+poll(self) : {B flightPhase(waitingAtRamp), ~B atc(tcg), ~B atc(tcd), B waiting(atc,tc)} <- +.lock, -poll(self), +polled(self), -.lock; " +
					
					
					 
					"+poll(self) : {B route(taxi,Num,Time,Fuel,Safety), B fuel(level,F), B atc(tcg,D), B flightPhase(waitingAtRamp), B waiting(planner,reqTaxiRoute)} <- +.lock, -poll(self), -waiting(planner,reqTaxiRoute), send(planner,enactRoute,taxi,Num), +waiting(planner,enactRoute,taxi), +polled(self), -.lock; " +
					"+poll(self) : {~B route(taxi,Num,Time,Fuel,Safety), B fuel(level,F), B atc(tcg,D), B flightPhase(waitingAtRamp), B waiting(planner,reqTaxiRoute)} <- +.lock, -poll(self), +polled(self), -.lock; " +
					...
					"+poll(self) : {B enactRoute(taxi,Num), B waiting(planner,enactRoute,taxi), B flightPhase(waitingAtRamp), B destination(D), B position(Lat,Lon,Alt), B fuel(level,F), G missionComplete [achieve]} <- +.lock, -waiting(planner,enactRoute,taxi), -poll(self), send(planner,reqRoute,D,Lat,Lon,F), +waiting(planner,reqRoute), +polled(self), -.lock; " 	+
					"+poll(self) : {~B enactRoute(taxi,Num), B waiting(planner,enactRoute,taxi), B flightPhase(waitingAtRamp), G missionComplete [achieve]} <- +.lock, -poll(self), +polled(self), -.lock; " 	+
					
					"+poll(self) : {B route(cruise,Num,Time,Fuel,Safety), B waiting(planner,reqRoute), B fuel(level,F), B destination(D), B flightPhase(waitingAtRamp)} <- +.lock, -poll(self), -waiting(planner,reqRoute), send(planner,enactRoute,cruise,Num), +waiting(planner,enactRoute), +polled(self), -.lock; " +
					"+poll(self) : {~B route(cruise,Num,Time,Fuel,Safety), B waiting(planner,reqRoute), B fuel(level,F), B destination(D), B flightPhase(waitingAtRamp)} <- +.lock, -poll(self), +polled(self), -.lock; " +
					
					// if route is being enacted (on take off) by the planner, then send t/o request to atc, and poll sensors while waiting
					"+poll(self) : {B enactRoute(cruise,Num), B atc(tcg,R), B waiting(planner,enactRoute), B flightPhase(waitingAtRamp), G missionComplete [achieve]} <- +.lock, -poll(self), -waiting(planner,enactRoute), -flightPhase(waitingAtRamp), -atc(tcg,R), +flightPhase(taxying), send(veh,updatefp,taxying), +polled(self), -.lock; " 	+
					"+poll(self) : {~B enactRoute(cruise,Num), B atc(tcg,R), B waiting(planner,enactRoute), B flightPhase(waitingAtRamp), G missionComplete [achieve]} <- +.lock, -poll(self), +polled(self), -.lock; " 	+
										
					"+poll(self) : {B veh(status,atRunwayHoldPosition), B flightPhase(taxying), G missionComplete [achieve]} <- +.lock, -veh(status,atRunwayHoldPosition), -poll(self), -flightPhase(taxying), +flightPhase(holding), send(veh,updatefp,holding), send(atc,reqluc), +waiting(atc,reqluc), +polled(self), -.lock; " +
					"+poll(self) : {~B veh(status,atRunwayHoldPosition), B waiting(atc,reqluc), B flightPhase(taxying), G missionComplete [achieve]} <- +.lock, -poll(self), +polled(self),  -.lock; " +
					
					
					"+poll(self)  : {B atc(lucg), B waiting(atc,reqluc), G missionComplete [achieve], B flightPhase(holding)} <- +.lock, -poll(self), -waiting(atc,reqluc), -atc(lucg), -flightPhase(holding), +flightPhase(lineup), send(veh,updatefp,lineup), +polled(self), -.lock; " +
					"+poll(self)  : {B atc(lucd), B waiting(atc,reqluc), G missionComplete [achieve], B flightPhase(holding)} <- +.lock, -poll(self), +polled(self), -.lock; " +
					
					"+poll(self) : {B veh(status,linedup), B flightPhase(lineup), G missionComplete [achieve]} <- +.lock, -poll(self), -veh(status,linedup), -flightPhase(lineup), +flightPhase(linedup), +waiting(atc,reqtoc), send(atc,reqtoc), +polled(self), -.lock; " +

					"+poll(self) : {B atc(tocg), B waiting(atc,reqtoc), G missionComplete [achieve], B flightPhase(linedup)} <- +.lock, -poll(self), -atc(tocg), -waiting(atc,reqtoc), -flightPhase(linedup), +flightPhase(takeOff), send(veh,updatefp,takeOff), +polled(self), -.lock; " +
					"+poll(self) : {B atc(tocd), B waiting(atc,reqtoc), G missionComplete [achieve], B flightPhase(linedup)} <- +.lock, -poll(self), -atc(tocd), send(atc,reqtoc), -.lock; " +

					
					"+poll(self) : {B veh(status,cruise), B flightPhase(takeOff)} <- +.lock, -poll(self), -veh(status,cruise), -flightPhase(takeOff), +flightPhase(cruise), +polled(self), -.lock;    " +
					"+poll(self) : {~B veh(status,cruise), B flightPhase(takeOff)} <- +.lock, -poll(self), +polled(self), -.lock;    " +
			...
			// Property: "Always: if an object (intruder aircraft) is approaching then eventually go into emergency avoid mode"
			String propertystring = "A( ||(    ~B(exec,das(objectApproaching))    , E(B(exec,flightPhase(emergencyAvoid))) ) )";

			
			MAS mas = (new GwendolenMASBuilder(masstring)).getMAS();

			// Set up a MCAPL controller and specification.
			MCAPLcontroller mccontrol = new MCAPLcontroller(mas,
					propertystring, outputlevel);
