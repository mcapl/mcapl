...
function INFO=RequestDataItem(stringReq)
%Agent Framework file to perform request for data item held on Physical 
%Engine by the Continuous Engine; counter part operating on the Physical 
%Engine is CommunicateWithCont

global Cont2Phys;
%send request for data item
Cont2Phys.write('Data');
%data bit desired
    ...
    if Cont2Phys.ready(),
       ...
       if strcmp(Cont2Phys.readLine,'NEWDATA'),
           ...
           for i=1:args,                    
                ...
                if strcmp(bit,'Does not exist'),error('Error in requested field');end
                Bit=str2num(bit)';
                INFO(i,1)=(Bit);
           end
